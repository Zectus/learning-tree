...
Wherever a formal definition appears — whether it's the central object of the whole document or a sub-piece inside one section — treat it as arrived at, not declared: why is it phrased exactly this way. Prove or derive each result at a rigor level matched to the topic — for foundational or subtle material, every non-trivial step should name what licenses it (which definition by name, which earlier result, which algebraic fact, which specific piece of evidence), with no "it can be shown that." For simpler topics, full step-by-step justification of every obvious move is unnecessary and just adds noise. The baseline rule: anything the material actually in front of the reader right now would make them wonder "but why?" about must be addressed — not anything a reader could in principle ask given where the document is eventually headed. Don't manufacture a question nobody at this exact point would have, and don't answer one early just because you know it's coming: a fact only becomes a live question once something already on the page makes someone care about it, not merely because it's logically adjacent to a result later in the tree. This includes flagging a not-yet-derived fact as a pending item or a "that will be shown/proved later" note — that narrates the document's own outline instead of just writing it, and it steals the surprise a later section is supposed to deliver on its own terms. If nothing in the current section's content needs a related fact, leave it unmentioned; it will get its actual treatment, unannounced, in the section that earns it. Apply the baseline rule with the most force to whichever single formula everything else in the document is actually built on — that's the one place an unjustified assertion does the most damage, since every later derivation inherits the gap silently. Every abstract object or claim introduced should be shown to actually work in a concrete case — asserting that it exists or holds is not enough. Once such a case has been worked through here, it's spent — see REUSED MATERIAL under QUESTION CONSTRUCTION RULES below for exactly what that rules out in the questions that follow.
...
- The one formula everything else depends on: does it have a real derivation on the page, or does the document just state what it is and move on?
- Every question's objects, genuinely new: for each question that follows a worked example or a just-established general method, would answering it take anything beyond relabeling what was already shown — a different kind of expression, not just different numbers inside the same shape?
