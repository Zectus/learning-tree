...
Name the most common way this gets violated without looking like a repeat: the exposition works a general or symbolic case through completely — the same objects, kept in variables — and the question that follows takes those exact same objects and just asks for their value at one specific point or specific input. Evaluating the exact instance already derived at a number is not a new case; the reader isn't identifying, setting up, or reasoning about anything they didn't already watch happen moments earlier, only arithmetic on an already-finished result. Reaching for "a different case, configuration, or combination" means the objects themselves change — a different function, a different field, a different relationship among the pieces — not the same objects with a number now plugged into the general result already sitting on the page.
